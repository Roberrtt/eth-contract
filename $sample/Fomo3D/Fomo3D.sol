...
contract F3Devents {
    ...
    event onNewName   // 玩家注册名称 （分配对应的 affiliate 信息）
    (
    ...
    event onEndTx   // 购买之后 或者 重载后 获取最新玩家信息
    (
    ...
    event onWithdrawAndDistribute  // 提现 并 分配 （在一轮结束的时候）
    (
        address playerAddress,
        bytes32 playerName,
        uint256 ethOut,
    ...
    event onBuyAndDistribute   // 结束购买，清零结束本轮 并分配
    (
        address playerAddress,
        bytes32 playerName,
        uint256 ethIn,
    ...
    // hit zero, and causes end round to be ran.
    event onReLoadAndDistribute    // 重载信息 并 分配
    (
        address playerAddress,
        bytes32 playerName,
        uint256 compressedData,
    ...
    event onAffiliatePayout  // 根据 affiliate 支付
    (
    ...
    event onPotSwapDeposit  // 奖池 交换 收益
    (
        uint256 roundID,
...
//====|=========================================================================
        ...
        public
    {
        ...
        if (_now > round_[_rID].end && round_[_rID].ended == false && round_[_rID].plyr != 0)  // 一轮结束的时候 
        {
            // set up our tx event data
            F3Ddatasets.EventReturns memory _eventData_;
            ...
            emit F3Devents.onWithdrawAndDistribute   // 提现并分配
            (
                msg.sender, 
                plyr_[_pID].name, 
                _eth, 
...
//=====================_|=======================================================
        ...
        private
    {
        ...
        // early round eth limiter   // 早期轮限制
        if (round_[_rID].eth < 100000000000000000000 && plyrRnds_[_pID][_rID].eth.add(_eth) > 1000000000000000000)  // 本轮总数< 100ETH，玩家> 1ETH
        {
            uint256 _availableLimit = (1000000000000000000).sub(plyrRnds_[_pID][_rID].eth);
            uint256 _refund = _eth.sub(_availableLimit);     // 退款
            plyr_[_pID].gen = plyr_[_pID].gen.add(_refund);  // 退款加入常规金额中
            _eth = _availableLimit;  // 本轮真正eth总数
        }
        ...
        if (_eth > 1000000000) 
        {
            ...
            // manage airdrops
            if (_eth >= 100000000000000000)   // 0.1 ETH
            {
                airDropTracker_++;
                if (airdrop() == true)
                {
                    ...
                    } else if (_eth >= 1000000000000000000 && _eth < 10000000000000000000) {   // 1-10ETH
                        // calculate prize and give it to winner
                    ...
                    } else if (_eth >= 100000000000000000 && _eth < 1000000000000000000) {  // 0.1-1 ETH
                        // calculate prize and give it to winner
...
//==============================================================================
     ...
     */
    function managePlayer(uint256 _pID, F3Ddatasets.EventReturns memory _eventData_)  // 转移之前未揭露的收益
        private
        returns (F3Ddatasets.EventReturns)
    {
     ...
     */
    function endRound(F3Ddatasets.EventReturns memory _eventData_)  // 一轮结束，管理分配 赢家和奖池
        private
        returns (F3Ddatasets.EventReturns)
    {
        // setup local rID
        ...
        // calculate our winner share, community rewards, gen share,    // 计算 赢家分配、社区分配，个人平分配
        // p3d share, and amount reserved for next pot  // p3d分配、 剩余量 、 下一次奖池数量
        uint256 _win = (_pot.mul(48)) / 100;   // 48% 给赢家
        uint256 _com = (_pot / 50);            // 2%  给社区
        uint256 _gen = (_pot.mul(potSplit_[_winTID].gen)) / 100;  // 奖池常规分配比例  根据team来
        uint256 _p3d = (_pot.mul(potSplit_[_winTID].p3d)) / 100;  // 奖池p3d分配比例  根据team来
        uint256 _res = (((_pot.sub(_win)).sub(_com)).sub(_gen)).sub(_p3d); // 奖池剩余量
        
        // calculate ppt for round mask
        uint256 _ppt = (_gen.mul(1000000000000000000)) / (round_[_rID].keys);  // 分配所占比例
        ...
        // pay our winner
        plyr_[_winPID].win = _win.add(plyr_[_winPID].win);   // 分配赢家
        
        // community rewards
        if (!address(Jekyll_Island_Inc).call.value(_com)(bytes4(keccak256("deposit()"))))
        {
            // This ensures Team Just cannot influence the outcome of FoMo3D with    // 这就保证了团队无法影响FoMo3D的结果
            // bank migrations by breaking outgoing transactions.
            // Something we would never do. But that's not the point.               // 这是我们永远不会做的。但这不是重点
            // We spent 2000$ in eth re-deploying just to patch this, we hold the   // 我们花了2000美元重新部署，只是为了修补，
            // highest belief that everything we create should be trustless.        // 我们坚信我们创造的一切都是不可信的
            // Team JUST, The name you shouldn't have to trust.                     // JUST团队，你不应该相信这个名字
            _p3d = _p3d.add(_com);
            _com = 0;
        }
        
        // distribute gen portion to key holders
        round_[_rID].mask = _ppt.add(round_[_rID].mask);   // 分配 gen 给key持有者
        ...
        _eventData_.newPot = _res;  // 下一次奖池初始值
     ...
     */
    function updateGenVault(uint256 _pID, uint256 _rIDlast)  // 转移未揭露的收益 到 个人收益，更新真正收益
        private 
    {
     ...
     */
    function updateTimer(uint256 _keys, uint256 _rID)   // 根据key来更新时间
        private
    {
        // grab time
        uint256 _now = now;
        ...
        if (_now > round_[_rID].end && round_[_rID].plyr == 0)
            _newTime = (((_keys) / (1000000000000000000)).mul(rndInc_)).add(_now);   // 增加一个key，增加rndInc_ 30s
        else
        ...
        returns(bool)
    {
        ...
        if((seed - ((seed / 1000) * 1000)) < airDropTracker_)  // 每次购买，享受空投的几率就会变大
            return(true);
        else
            return(false);
    }
